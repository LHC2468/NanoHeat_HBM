...
function geometry = createVias_con_stagger_switching(geometry, via_density, via_height, metal_line_width, metal_thickness, area_size, num_layers)
    ...
    for v_layer = 1:num_layers_via
        ...
        %Even number layers. Assign vias at the diagonal
        if rem(v_layer+3,2) == 0  
            via_number = length(intersect_x);
            for i=1:via_number
                if rem(i,round((1/via_density)*(num_lines_v_1+1))) == 1
                    x_pos  = intersect_x(i);
                    y_pos  = intersect_y(i);
        ...
        %Odd number layers. Shift vias for 1x pitch
        if rem(v_layer+3,2) == 1         
            via_number = length(intersect_x);
            
            % Assign the vias periodically [This is for SWITCHING]
            ...
            for i=1:via_number
                ...
                if rem(i-1-(cycle_num-1),round((1/via_density)*(num_lines_v_1+1))) == 1
