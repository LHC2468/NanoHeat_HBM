...
function geometry = createVias_isolated_shifting(geometry, via_density, via_height, metal_line_width, metal_thickness, area_size, num_layers)
    ...
    for v_layer = 1:num_layers_via
        ...
        % Repeated layer 1/4. Assign vias at the diagonal, skip lines
        if rem(v_layer+3,4) == 0
        ...
        % Repeated layer 2/4. Shift vias for 1x pitch, skip lines
        if rem(v_layer+3,4) == 1
            via_number = length(intersect_x);
            for i=1:via_number
        ...
        % Repeated layer 3/4. Shift vias for 1x pitch, skip lines
        if rem(v_layer+3,4) == 2
            via_number = length(intersect_x);

            % Assign the vias periodically [This is for SWITCHING, SKIPPING]
            ...
            for i=1:via_number
                ...
                if rem(i-1-(cycle_num-1),round((1/via_density)*2*(num_lines_v_1+1))) == 1
                    ...
                    if rem(i, num_lines_v_1*(num_lines_v-1)) == 0
        ...
        % Repeated layer 4/4. Shift vias for 1x pitch, skip lines
        if rem(v_layer+3,4) == 3
            via_number = length(intersect_x);
            ...
            for i=1:via_number
                % Via location is based on the number of cycle
                if rem(i-2-(cycle_num-1)-num_lines_v_1,round((1/via_density)*2*(num_lines_v_1+1))) == 1
